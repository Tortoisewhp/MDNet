...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize, mean=None, std=None, randomPeper=True,
        ...
        # get filenames
        depths_root='/home/wby/Desktop/irseg/train.txt'
        fp=open(depths_root,'r')
        allFileName=[line.strip("\n") for line in fp.readlines()]
        fp.close()
        self.images = [image_root +f+"_rgb.png" for f in allFileName]#.jpg
        self.depths = [image_root + f+"_th.png" for f in allFileName]  # .jpg
        self.gts = [gt_root + f+".png" for f in allFileName]
    ...
    def __getitem__(self, index):
        ...
        depth, gt1 = self.aug_transform(image=np.asarray(depth), mask=np.asarray(gt1)).values()
        depth, gt1 = albumentations.PadIfNeeded(*image_size[::-1], border_mode=0)(image=depth, mask=gt1).values()
        depth, gt1 = albumentations.RandomCrop(*image_size[::-1])(image=depth, mask=gt1).values()
        ...
        depth = colorEnhance(Image.fromarray(depth))
...
class test_dataset(data.Dataset):
    def __init__(self, image_root, gt_root, testsize, mean=None, std=None):
        ...
        depths_root = '/home/wby/Desktop/irseg/test.txt'
        fp = open(depths_root, 'r')
        allFileName = [line.strip("\n") for line in fp.readlines()]
        fp.close()
        self.images = [image_root + f + "_rgb.png" for f in allFileName]  # .jpg
    ...
    def __getitem__(self, item):
        ...
        #image=torch.cat([image, depth],dim=0)#.unsqueeze(0)

        gt = self.gt_transform(self.binary_loader(self.gts[item]))#.cpu()
