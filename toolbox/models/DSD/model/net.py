...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim = None, groups = 8):
        ...
        self.block1 = Block1(dim, dim_out, groups = groups)
        self.block2 = Block1(dim_out, dim_out, groups = groups)
...
class PyramidVisionTransformerImpr(nn.Module):
    def __init__(self, img_size=480, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        # self.conv1=nn.Conv2d(6,3,1,1,0)

    def forward_features(self, x, timesteps, cond_img):
        #print(x.shape, timesteps.shape, cond_img.shape)#torch.Size([4, 1, 416, 416]) torch.Size([4]) torch.Size([4, 3, 416, 416])
        #print(x.shape)
        #x=self.conv1(x)
...
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=1, **kwargs):#mask_chans=0
        super(net, self).__init__()
        self.class_num = class_num
        self.backbone = pvt_v2_b0(in_chans=3, mask_chans=mask_chans)#[32, 64, 160, 256]  [64, 128, 320, 512]
        self.decode_head = Decoder(dims=[32, 64, 160, 256], dim=128, class_num=class_num, mask_chans=mask_chans)
        #self._init_weights()  # load pretrain

    def forward(self, x, timesteps, cond_img,cond_dep):
        #print(x.shape)
        featuresR = self.backbone(x, timesteps, cond_img)
        featuresD = self.backbone(x, timesteps, cond_dep)
        features=[featuresR[i]+featuresD[i] for i in range(4)]
    ...
    def sample_unet(self, x, timesteps, cond_img,cond_dep):
        return self.forward(x, timesteps, cond_img,cond_dep)

    def extract_features(self, cond_img,cond_dep):
        # do nothing
        return cond_img,cond_dep
...
if __name__ == '__main__':
    img = torch.randn(1, 3, 480,640).cuda(0)
    depth = torch.randn(1, 3,480,640).cuda(0)
    out = torch.randn(1, 1, 480,640).cuda(0)
    time=torch.randn(1).cuda(0)
    print(img.shape,out.shape,time.shape)
    model = net().cuda(0)