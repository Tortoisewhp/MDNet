...
from CamoDiffusion.utils.msg import runMsg
...
class Trainer(object):
    def __init__(
        ...
        #self.conv1 = nn.Conv2d(6, 3, 1, 1, 0).cuda()
        self.running_metrics_test = runMsg()
        self.globalIoU,self.globalBER=0.0,50.0
    ...
    def val(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        ...
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            data['image']=self.conv1(data['image'])
    ...
    def val_time_ensemble(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        ...
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            ...
            #print('gt.shape', gt[0].shape, '---', len(gt))
            gt = [np.array(x, np.float32) for x in gt.cpu()]
            #print('gt.shape',gt[0].shape,'---',len(gt))#(1024, 1280) --- 3
            #print(gt[0].max())
            #gt = [x / x.max() + 1e-8 for x in gt]
            image = image.to(device).squeeze(1)
            ...
            #print('ensem_res.shape',ensem_res.shape)
            #print('ensem_res.shape',ensem_res[0].shape,'---',len(ensem_res))#(1024, 1280) --- 3
            for count in range(len(gt)):
                self.running_metrics_test.update(torch.tensor(gt[count]), torch.tensor(ensem_res[count]))

            ensemble_maes += [cal_mae(g, r, thresholding, save_to, n) for g, r, n in zip(gt, ensem_res, name)]

        metrics = self.running_metrics_test.get_scores()

        test_iou = metrics["iou: "].item() * 100
        test_ber = metrics["ber: "].item() * 100
        test_mae = metrics["mae: "].item()
        print('IoU:',test_iou,' BER:',test_ber,' MAE:',test_mae)
        ...
        return ensemble_maes, _best_mae,test_iou,test_ber,test_mae
    ...
    def train(self):
        ...
        for epoch in range(self.cur_epoch, self.train_num_epoch):
            ...
            mae, best_mae,test_iou,test_ber,test_mae = self.val_time_ensemble(self.model, self.test_loader, accelerator)
            self.logger.info(f'Epoch:{epoch}/{self.train_num_epoch} mae: {mae:.4f}({best_mae:.4f})')
            accelerator.log({'mae': mae, 'best_mae': best_mae})
            # if mae == best_mae:
            #     self.save("best")
            if test_iou>self.globalIoU:
                self.globalIoU=test_iou
                self.save("IoU")
            if test_ber<self.globalBER:
                self.globalBER=test_ber
