...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img,cond_dep ,seg=None, extra_cond=None, *args, **kwargs):
        ...
        return self.p_losses(img, times, cond_img,cond_dep, seg, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times, cond_img,cond_dep, seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        model_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img,cond_dep)
    ...
    @torch.no_grad()
    def sample(self, cond_img,cond_dep, extra_cond=None, verbose=True):
        b, c, h, w = cond_img.shape
        extra_cond = default(extra_cond, torch.zeros((b, self.extra_channels, h, w), device=self.device))
        return self.p_sample_loop((b, self.channels, 480, 640),
                                  cond_img,cond_dep,
                                  extra_cond=extra_cond, verbose=verbose)

    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img,cond_dep, extra_cond, verbose=True):
        self.history = []
        img = torch.randn(shape, device=self.device)
        conditioning_features,cond_dep = self.model.extract_features(cond_img,cond_dep)
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img = self.p_sample(img, conditioning_features,cond_dep, extra_cond, times, times_next)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond,cond_dep, extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance = self.p_mean_variance(x=x, cond=cond,cond_dep=cond_dep, extra_cond=extra_cond, time=time,
                                                          time_next=time_next)
    ...
    def p_sample_g(self, x, cond,cond_dep, extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device
    ...
    def p_mean_variance(self, x, cond,cond_dep, extra_cond, time, time_next):
        ...
        #print(x.shape, extra_cond.shape)
        pred = self.model.sample_unet(torch.cat([x, extra_cond], dim=1),
                                      batch_log_snr, cond,cond_dep)
