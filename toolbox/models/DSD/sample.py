...
def get_loader(cfg):
    cod10k_test_dataset = instantiate_from_config(cfg.test_dataset.NC4K)#instantiate_from_config(cfg.test_dataset.COD10K)
    camo_test_dataset = instantiate_from_config(cfg.test_dataset.NC4K)#instantiate_from_config(cfg.test_dataset.CAMO)
    chameleon_test_dataset = instantiate_from_config(cfg.test_dataset.NC4K)#instantiate_from_config(cfg.test_dataset.CHAMELEON)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str, default='/home/MDNet/model-best.pt')
    parser.add_argument('--fp16', action='store_true')
    parser.add_argument('--results_folder', type=str, default='./MDNet')
    parser.add_argument('--num_epoch', type=int, default=150)
    parser.add_argument('--batch_size', type=int, default=4)
    parser.add_argument('--gradient_accumulate_every', type=int, default=1)
    parser.add_argument('--num_workers', type=int, default=8)
    parser.add_argument('--num_sample_steps', type=int, default=10)
    parser.add_argument('--target_dataset', nargs='+', type=str, default=['NC4K'])#'CAMO', 'COD10K', 'CHAMELEON',
    ...
    for dataset, dataset_name in target_dataset:
        trainer.model.eval()
        mask_path = Path(cfg.test_dataset.NC4K.params.image_root).parent.parent#CAMO
